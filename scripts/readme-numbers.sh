#!/usr/bin/env bash
# Numbers this repository states about itself have to be true. The suite
# grows in commits that never open README.md, which is what this makes
# impossible.
#
# Copied verbatim from vouchryx scripts/readme-numbers.sh.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# `func Test...` the suite has. Checked separately because the two numbers
# are allowed to differ and a single comparison could not catch either
# drifting on its own.
